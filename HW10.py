...
def minVarLaplacian(image, threshold):
    ...
    for i in range(1, row + 1):
        for j in range(1, col + 1):
            r = 2 * int(pad[i - 1, j - 1]) + (-1) * int(pad[i - 1, j]) + 2 * int(pad[i - 1, j + 1]) + (-1) * int(
                pad[i, j - 1]) \
                + (-1) * int(pad[i, j + 1]) + 2 * int(pad[i + 1, j - 1]) + (-1) * int(pad[i + 1, j]) + 2 * int(
                pad[i + 1, j + 1]) \
                - 4 * int(pad[i, j])
            r = r / 3
            if r >= threshold:
...
cv.imwrite('results/HW10_part(a)_image_lapla_mask1.jpg', lapla_mask1) #這樣寫才可以一次存全部的照片

lapla_mask2 = zeroCross(laplacianTwo(img, 15), 1)
cv.imshow('lapla_mask2', lapla_mask2)
cv.imwrite('results/HW10_part(b)_image_lapla_mask2.jpg', lapla_mask2)

min_lapla = zeroCross(minVarLaplacian(img, 20), 1)
cv.imshow('min_lapla', min_lapla)
cv.imwrite('results/HW10_part(c)_image_min_lapla.jpg', min_lapla)

lapla_gaussian = zeroCross(laplaceOfGaussian(img, 3000), 1)
cv.imshow('lapla_gaussian', lapla_gaussian)
cv.imwrite('results/HW10_part(d)_image_lapla_gaussian.jpg', lapla_gaussian)

dif_gaussian = zeroCross(deferenceOfGaussian(img, 1), 1)
cv.imshow('dif_gaussian', dif_gaussian)
cv.imwrite('results/HW10_part(e)_image_dif_gaussian.jpg', dif_gaussian)
cv.waitKey(0)