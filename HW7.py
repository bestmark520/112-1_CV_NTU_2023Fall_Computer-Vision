...
#先單獨存thresholding and downsampling
image2 = cv.imread('lena.bmp', cv.IMREAD_GRAYSCALE)
image2 = main2(main1(image2, 128))
cv.imwrite(os.path.join(output_directory, 'HW7_part(a)_image_thresholding and downsampling.jpg'), image2)
cv.imshow('image_thinning', image2)
cv.waitKey(0)

image = cv.imread('lena.bmp', cv.IMREAD_GRAYSCALE)
image = main2(main1(image, 128))  # 先二值化在取sample
row, col = image.shape
...
cv.imwrite(os.path.join(output_directory, 'HW7_part(b)_image_thinning.jpg'), image)
cv.imshow('image_thinning', image)
cv.waitKey(0)